...
const controlPacketStart = source.indexOf('static AppMessageResult send_control_packet(');
const controlPacketEnd = source.indexOf('\n}', controlPacketStart);
const controlPacket = source.slice(controlPacketStart, controlPacketEnd);
assert(controlPacket.includes('message->type == MSG_REQUEST_SNAPSHOT'));
assert(controlPacket.includes('dict_write_uint32(iterator, MESSAGE_KEY_SESSION_ID, s_session_id)'));
