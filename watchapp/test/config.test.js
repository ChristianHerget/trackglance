...
assert.deepStrictEqual(config.configResultMessage({0:3,1:9,35:'0.1.9',33:5,4:7}),{id:5,result:7});
assert.strictEqual(config.configResultMessage({0:3,1:9,35:'0.1.6',33:5,4:7}),null);
assert.strictEqual(config.configResultMessage({0:3,1:9,35:'0.1.9',33:5,4:'7'}),null);
...
assert(config.validHeartRateMessage({0:3,1:8,35:'0.1.9',7:1,38:0,6:1000,37:123}));
assert(!config.validHeartRateMessage({0:2,1:8,35:'0.1.9',7:1,38:0,6:1000,37:123}));
assert(!config.validHeartRateMessage({0:3,1:8,35:'0.1.9',7:1,38:0,6:1000,37:251}));
...
handlers.appmessage({payload:{0:3,1:9,35:'0.1.9',33:startupTransferId,4:0}});
...
sent[sentCount-1].ok();
handlers.appmessage({payload:{0:3,1:6,4:0,35:'0.1.9',33:77,30:0,31:1,32:'Wandern\nRadfahren\nLaufen'}});
...
global.openedSettingsURL = null;
handlers.appmessage({payload:{PROTOCOL_VERSION:3,MESSAGE_TYPE:6,RESULT:0,APP_VERSION:'0.1.9',TRANSFER_ID:79,
...
handlers.appmessage({payload:{0:3,1:6,4:3,35:'0.1.9',33:80,30:0,31:1,32:''}});
...
handlers.appmessage({payload:{0:3,1:6,4:0,35:'0.1.9',33:82,30:0,31:1,32:'Wandern'}});
global.openedSettingsURL = null;
handlers.showConfiguration();
lifecycleHtml=decodeURIComponent(global.openedSettingsURL);
...
function configAck(id,result){handlers.appmessage({payload:{0:3,1:9,35:'0.1.9',33:id,4:result}});}
...
assert.strictEqual(migratedPending.release,'0.1.9');
assert.strictEqual(migratedPending.format,2);
...
const tamperedLineage=JSON.stringify({protocol:3,release:'0.1.9',format:2,items:[{
...
const brokenTwoItemLineage=JSON.stringify({protocol:3,release:'0.1.9',format:2,items:[{
...
const boundedState={protocol:3,release:'0.1.9',format:2,items:[{
...
storage['configPending.v3']=JSON.stringify({protocol:3,release:'0.1.9',format:2,items:[{
