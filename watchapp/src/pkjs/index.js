...
(function (root) {
  ...
  var uiEnglish={general:'General',generalSettings:'General settings',page:'Page',pages:'pages',inactivePage:'Inactive page',customName:'Custom page name',resetActivity:'Reset activity',resetGeneral:'Reset General',confirmActivityReset:'Reset this activity to its automatic page?',confirmGeneralReset:'Reset General settings?',finalPage:'At least one page must remain active.',moveUp:'Move up',moveDown:'Move down',position:'position',of:'of',openActivity:'Edit activity',grabbed:'Grabbed',dropped:'Dropped',dragCancelled:'Move cancelled',pageFull:'This page already has six metrics.',metricUsed:'This metric is already on that page.'};
  ...
  var localizedDrag={
    de:{grabbed:'Aufgenommen',dropped:'Abgelegt',dragCancelled:'Verschieben abgebrochen',pageFull:'Diese Seite enthält bereits sechs Messwerte.',metricUsed:'Dieser Messwert ist bereits auf dieser Seite.'},
    fr:{grabbed:'Élément saisi',dropped:'Élément déposé',dragCancelled:'Déplacement annulé',pageFull:'Cette page contient déjà six mesures.',metricUsed:'Cette mesure figure déjà sur cette page.'},
    es:{grabbed:'Elemento recogido',dropped:'Elemento soltado',dragCancelled:'Movimiento cancelado',pageFull:'Esta página ya contiene seis métricas.',metricUsed:'Esta métrica ya está en esa página.'},
    it:{grabbed:'Elemento afferrato',dropped:'Elemento rilasciato',dragCancelled:'Spostamento annullato',pageFull:'Questa pagina contiene già sei metriche.',metricUsed:'Questa metrica è già presente nella pagina.'},
    pt:{grabbed:'Item selecionado',dropped:'Item colocado',dragCancelled:'Movimento cancelado',pageFull:'Esta página já contém seis métricas.',metricUsed:'Esta métrica já está nesta página.'},
    zh_CN:{grabbed:'已选中',dropped:'已放置',dragCancelled:'已取消移动',pageFull:'此页面已有六个指标。',metricUsed:'此页面已有该指标。'},
    zh_TW:{grabbed:'已選取',dropped:'已放置',dragCancelled:'已取消移動',pageFull:'此頁面已有六個指標。',metricUsed:'此頁面已有該指標。'}
  };
  ...
  function defaultsFor() { return {schema: 3, theme: 'dark', watchHrToLocus: false, heartRateIntervalSeconds: 5, activities: []}; }
  ...
  function validate(config, allowLegacy) {
    ...
    if (!config || config.schema !== 3 || (config.theme !== 'dark' && config.theme !== 'light') || typeof config.watchHrToLocus !== 'boolean' || integer(config.heartRateIntervalSeconds, 1, 60) === null || !Array.isArray(config.activities)) return false;
    var activityIds = {}, pageIds = {};
    return config.activities.every(function (activity) {
      if (!activity || (!validLocusId(activity.locusId) && !(allowLegacy && activity.locusId === '')) || !validLocus(activity.locusName) || activityIds[activity.locusId] || !Array.isArray(activity.pages) || activity.pages.length !== LIMIT.pages || !activity.pages.some(function(p){return p&&p.metrics&&p.metrics.length;})) return false;
  ...
  function canonicalObject(config) {
    return {schema:3,theme:config.theme,watchHrToLocus:config.watchHrToLocus,heartRateIntervalSeconds:config.heartRateIntervalSeconds,activities:config.activities.map(function(a){return{locusId:a.locusId,locusName:a.locusName,pages:a.pages.map(function(p){return{id:p.id,type:p.type,name:p.name,metrics:p.metrics.slice()};})};})};
  }
  ...
  function reconcile(config, catalog, lang) {
    ...
    catalog.forEach(function (item) {
      ...
      var a=byId[item.id];if(!a)a={locusId:item.id,locusName:item.name,pages:defaultPages(item.name)};
  ...
  function add(config,locusId,index,metric){var g=activity(config,locusId),p=g&&g.pages[index];if(!p||p.metrics.length>=LIMIT.metrics||integer(metric,1,22)===null||p.metrics.indexOf(metric)>=0)return false;p.metrics.push(metric);return true;}
  ...
  function resetActivity(config,locusId){var g=activity(config,locusId);if(!g)return false;g.pages=defaultPages(g.locusName);return true;}
  ...
  function projection(config,locusId,lang){var group=activity(config,locusId);if(!group)return null;var fp=fingerprints(config,lang),active=group.pages.filter(function(p){return p.metrics.length;});return[config.theme,config.watchHrToLocus?'1':'0',config.heartRateIntervalSeconds,locusId,fp.a,fp.b].join('|')+'\n'+active.map(function(p,i){return[p.name===null?automaticName(lang,i+1):p.name,p.metrics.join(','),p.id].join('|');}).join('\n');}
  ...
  function dragEnhancement(){
    return [
      '<style>.drag-source{display:none!important}.drag-ghost{position:fixed;z-index:10;margin:0!important;pointer-events:none;opacity:.94;box-shadow:0 12px 28px rgba(15,23,42,.32);outline:2px solid var(--primary);overflow:hidden}.drag-placeholder{border:2px dashed var(--primary);border-radius:10px;background:var(--bg);margin:8px 0}.metric-placeholder{margin-top:8px}.page.drop-target{outline:2px solid var(--primary);outline-offset:2px}.page.drop-unavailable{opacity:.55;outline:2px dashed var(--border);outline-offset:2px}.drag-message{margin:0 4px 8px;color:var(--text);font-size:14px}.handle[aria-pressed=true]{background:var(--primary);color:var(--on-primary)}</style>',
      '<script>(function(){',
      'var drag=null,keyboard=null,ignoreMouseUntil=0,EDGE=48;',
      ...
      'function positionGhost(p){drag.last=p;drag.ghost.style.left=Math.round(p.x-drag.offsetX)+"px";drag.ghost.style.top=Math.round(p.y-drag.offsetY)+"px";movePlaceholder(p)}',
      'function scrollStep(){if(!drag)return;var y=drag.last.y,height=window.innerHeight||document.documentElement.clientHeight,amount=0;if(y<EDGE)amount=-Math.min(16,Math.ceil((EDGE-y)/3));else if(y>height-EDGE)amount=Math.min(16,Math.ceil((y-height+EDGE)/3));if(amount){window.scrollBy(0,amount);movePlaceholder(drag.last)}drag.frame=(window.requestAnimationFrame||function(fn){return setTimeout(fn,16)})(scrollStep)}',
      'function begin(e,input,spec,button){if(drag||keyboard||input==="mouse"&&Date.now()<ignoreMouseUntil||input==="mouse"&&e.button!==0)return;if(e.cancelable)e.preventDefault();var p=point(e),row=button.closest(".reorder-row"),rect=row.getBoundingClientRect(),ghost=row.cloneNode(true),placeholder=document.createElement("div");drag={kind:spec.kind,pageIndex:spec.pageIndex,pageId:spec.pageId,metricIndex:spec.metricIndex,value:spec.value,label:spec.label,row:row,ghost:ghost,placeholder:placeholder,input:input,offsetX:p.x-rect.left,offsetY:p.y-rect.top,last:p,targetIndex:spec.metricIndex===undefined?spec.pageIndex:spec.metricIndex,targetPageId:spec.pageId,valid:true,reason:""};placeholder.className="drag-placeholder "+(spec.kind==="metric"?"metric-placeholder":"page-placeholder");placeholder.style.height=Math.max(44,rect.height)+"px";ghost.classList.add("drag-ghost");ghost.style.width=Math.max(44,rect.width)+"px";ghost.style.height=Math.max(44,rect.height)+"px";row.parentNode.insertBefore(placeholder,row);row.classList.add("drag-source");document.body.appendChild(ghost);button.classList.add("dragging");report(T.grabbed+" "+spec.label,false);positionGhost(p);scrollStep()}',
      'function matching(input){return drag&&drag.input===input}',
      'function move(e,input){if(!matching(input))return;if(e.cancelable)e.preventDefault();positionGhost(point(e))}',
      'function cleanup(){if(!drag)return;clearTargets();if(drag.frame){if(window.cancelAnimationFrame)window.cancelAnimationFrame(drag.frame);else clearTimeout(drag.frame)}if(drag.ghost.parentNode)drag.ghost.parentNode.removeChild(drag.ghost);if(drag.placeholder.parentNode)drag.placeholder.parentNode.removeChild(drag.placeholder);drag.row.classList.remove("drag-source");var buttons=document.querySelectorAll(".handle.dragging");for(var i=0;i<buttons.length;i++)buttons[i].classList.remove("dragging")}',
      'function finish(e,input,cancel){if(!matching(input))return;if(e&&e.cancelable)e.preventDefault();if(input==="touch")ignoreMouseUntil=Date.now()+700;var state=drag,failed=!state.valid,reason=state.reason;cleanup();drag=null;if(!cancel&&!failed){if(state.kind==="page"){var moved=draft.pages.splice(state.pageIndex,1)[0];draft.pages.splice(state.targetIndex,0,moved)}else{var source=pageById(state.pageId),destination=pageById(state.targetPageId),metric=source.metrics.splice(state.metricIndex,1)[0];destination.metrics.splice(state.targetIndex,0,metric)}}drawPages();if(cancel){report(T.dragCancelled,false);return}if(failed){report(reason,true);return}if(state.kind==="page")focusHandle("page",state.targetIndex);else focusHandle("metric-"+pageIndex(state.targetPageId),state.targetIndex);announce(T.dropped+" "+state.label,state.targetIndex,state.kind==="page"?draft.pages.length:pageById(state.targetPageId).metrics.length)}',
      'function bind(button,spec){function startPointer(e){begin(e,"pointer",spec,button)}function startTouch(e){begin(e,"touch",spec,button)}function startMouse(e){begin(e,"mouse",spec,button)}button.addEventListener("pointerdown",startPointer,false);button.addEventListener("touchstart",startTouch,false);button.addEventListener("mousedown",startMouse,false)}',
      ...
      'function keyboardStart(spec){keyboard={kind:spec.kind,pageIndex:spec.pageIndex,pageId:spec.pageId,metricIndex:spec.metricIndex,value:spec.value,label:spec.label,snapshot:copy(draft.pages)};keyboardFocus();report(T.grabbed+" "+spec.label,false)}',
      ...
      'controls=function(spec,label){var b=document.createElement("button");spec.label=label;b.className="handle";b.type="button";b.textContent="⠿";b.dataset.key=spec.kind==="page"?"page":"metric-"+pageIndex(spec.pageId);b.dataset.index=spec.kind==="page"?spec.pageIndex:spec.metricIndex;b.setAttribute("aria-label",T.drag+" "+label);b.setAttribute("aria-pressed","false");b.onkeydown=function(e){var activation=e.key==="Enter"||e.key===" "||e.key==="Spacebar";if(activation){e.preventDefault();if(!keyboard)keyboardStart(spec);else keyboardFinish(false);return}if(e.key==="Escape"&&keyboard){e.preventDefault();keyboardFinish(true);return}if(keyboard&&(e.key==="ArrowUp"||e.key==="ArrowDown")){e.preventDefault();keyboardMove(e.key==="ArrowUp"?-1:1)}};bind(b,spec);return b};',
