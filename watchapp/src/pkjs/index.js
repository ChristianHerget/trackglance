...
(function (root) {
  ...
  var uiEnglish={general:'General',page:'Page',pages:'pages',inactivePage:'Inactive page',customName:'Custom page name',resetActivity:'Reset activity',resetGeneral:'Reset General',confirmActivityReset:'Reset this activity to its automatic page?',confirmGeneralReset:'Reset General settings?',finalPage:'At least one page must remain active.',moveUp:'Move up',moveDown:'Move down',openActivity:'Edit activity'};
  ...
  function settingsPage(config,profiles,lang,state,notice,supportsHeartRate){
    ...
    var css=':root{color-scheme:light dark;--bg:#F4FBF6;--surface:#fff;--text:#1E293B;--muted:#64748B;--border:#CBD5E1;--primary:#006C4C;--on-primary:#fff}*{box-sizing:border-box}@media(prefers-color-scheme:dark){:root{--bg:#0F172A;--surface:#1E293B;--text:#F1F5F9;--muted:#94A3B8;--border:#475569;--primary:#34D399;--on-primary:#052E24}}body{margin:0;background:var(--bg);color:var(--text);font:16px system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}main{padding:12px 12px 92px}.hidden{display:none!important}.top{display:flex;align-items:center;justify-content:space-between;margin:4px 0 12px}.top h1{font-size:22px;margin:0}.icon,.handle,.remove{min-width:44px;width:44px}.card{background:var(--surface);border:1px solid var(--border);border-radius:12px;margin:8px 0;padding:12px}.activity{display:flex;width:100%;align-items:center;text-align:left;gap:12px}.activity .label{flex:1}.activity small,.muted{color:var(--muted)}button,input,select{min-height:44px;border:1px solid var(--border);border-radius:8px;background:var(--surface);color:var(--text);font:inherit;padding:8px}button:focus-visible,input:focus-visible,select:focus-visible{outline:3px solid var(--primary);outline-offset:2px}.primary{background:var(--primary);color:var(--on-primary);border-color:var(--primary)}.notice{font-size:14px;color:var(--muted);margin:0 4px 12px}.bar{position:fixed;display:flex;gap:8px;bottom:0;left:0;right:0;padding:12px;background:var(--surface);border-top:1px solid var(--border);z-index:3}.bar button{flex:1}.page{padding:0;overflow:hidden}.page-head{display:flex;align-items:center;gap:8px;padding:12px}.page-body{padding:0 12px 12px}.page-title{flex:1;min-width:0}.page-title input{width:100%}.badge{white-space:nowrap;border-radius:999px;background:var(--bg);padding:4px 8px}.metric{display:flex;gap:8px;align-items:center;margin-top:8px}.metric select{flex:1;min-width:0}.add{width:100%;margin-top:8px}.field{display:block;margin:12px 0}.field select,.field input[type=number]{width:100%;margin-top:4px}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important}}';
    var body='<!doctype html><html lang="'+l+'"><meta name="viewport" content="width=device-width,initial-scale=1"><style>'+css+'</style><body>'+
      '<main id="overview"><div class="top"><h1>'+safe(t.activities)+'</h1><button id="generalOpen" class="icon" aria-label="'+safe(t.general)+'">&#9881;</button></div><p class="notice">'+safe(notice||t[state]||'')+'</p><div id="activities"></div></main>'+
