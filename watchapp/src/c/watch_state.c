...
bool watch_snapshot_epoch_allowed(bool current_received, uint32_t current_epoch,
...
bool watch_transfer_serial_may_start(const WatchTransferSerialFloor *floor, int32_t candidate,
...
bool watch_transfer_serial_reserve_persistent(uint32_t key, int32_t seed, int32_t *reserved) {
...
bool watch_profile_transfer_join(const WatchProfileTransfer *transfer, char *buffer,
                                 size_t buffer_size, size_t maximum_length, size_t *joined_length) {
  if (!transfer || !buffer || !buffer_size || !joined_length || transfer->chunk_count < 1 ||
      transfer->chunk_count > WATCH_PROFILE_MAX_CHUNKS || maximum_length >= buffer_size) {
    return false;
  }
  size_t length = 0;
  for (int i = 0; i < transfer->chunk_count; i++) {
    ...
    if (!transfer->received[i] || source >= buffer_size || part_length > buffer_size - source ||
        part_length > maximum_length - length) {
      return false;
    }
...
static bool chunk_matches(const WatchProfileTransfer *transfer, const char *buffer, int index,
...
WatchProfileTransferOutcome watch_profile_transfer_accept(WatchProfileTransfer *transfer,
                                                          char *buffer, size_t buffer_size,
                                                          int32_t id, int index, int count,
                                                          int result, const char *data,
                                                          size_t length, uint32_t uptime_seconds) {
  if (!transfer) return WATCH_PROFILE_TRANSFER_INVALID;
  if (id < 0 || index < 0 || count < 1 || count > WATCH_PROFILE_MAX_CHUNKS || index >= count ||
      !data || length > WATCH_PROFILE_CHUNK_BYTES) {
    if (id == transfer->id) watch_profile_transfer_reset(transfer);
    return WATCH_PROFILE_TRANSFER_INVALID;
  }
  const size_t offset = (size_t)index * WATCH_PROFILE_CHUNK_BYTES;
  if (!buffer || buffer_size < 1 || offset >= buffer_size || length > buffer_size - 1 - offset) {
    if (id == transfer->id) watch_profile_transfer_reset(transfer);
    return WATCH_PROFILE_TRANSFER_INVALID;
  }
