...
enum { MSG_SNAPSHOT=1, MSG_COMMAND=2, MSG_COMMAND_RESULT=3, MSG_REQUEST_SNAPSHOT=4,
  MSG_CONFIG_CHUNK=5, MSG_PROFILE_LIST_CHUNK=6, MSG_REQUEST_PROFILE_LIST=7, MSG_HEART_RATE=8 };
...
enum { METRIC_ELAPSED=1, METRIC_MOVING_TIME=2, METRIC_DISTANCE=3, METRIC_MOVING_DISTANCE=4,
  ...
  METRIC_MAX_POWER=20, METRIC_ENERGY=21, METRIC_CURRENT_HR=22 };
...
typedef struct {
  ...
  int32_t avg_hr, max_hr, current_hr, avg_cadence, max_cadence, avg_power, max_power, energy;
} Snapshot;
...
static bool s_watch_hr_to_locus, s_health_subscribed, s_outbox_busy, s_hr_pending;
static uint8_t s_heart_rate_interval=5;
static uint32_t s_hr_sequence, s_last_hr_sent;
static int32_t s_pending_hr;
#if defined(PBL_PLATFORM_EMERY)
static AppTimer *s_health_timeout;
#endif
static bool s_health_notice_shown;
static int s_pending_message_type, s_pending_message_command;
...
static void defaults(void) {
  ...
  s_dark=true; s_selected=0; s_profile_count=2;s_watch_hr_to_locus=false;s_heart_rate_interval=5;
  copy_name(s_profiles[0].name,tr("Walking","Gehen")); copy_locus_name(s_profiles[0].locus,tr("Walking","Gehen"));
  uint8_t h[]={1,3,5,6,10,22}; memcpy(s_profiles[0].metrics,h,6); s_profiles[0].count=6;
  copy_name(s_profiles[1].name,tr("Cycling","Radfahren")); copy_locus_name(s_profiles[1].locus,tr("Cycling","Radfahren"));
  uint8_t c[]={1,3,5,6,7,22}; memcpy(s_profiles[1].metrics,c,6); s_profiles[1].count=6;
...
static bool parse_config(char *data) {
  ...
  bool watch_hr=watch_hr_text&&atoi(watch_hr_text)==1;
  int interval=interval_text?atoi(interval_text):5;if(interval<1||interval>60)return false;
  while((line=next_token(&line_cursor,'\n')) && count<MAX_PROFILES) {
    ...
    while(m&&parsed[count].count<MAX_SLOTS){int id=atoi(m);if(id<1||id>22)return false;for(int j=0;j<parsed[count].count;j++)if(parsed[count].metrics[j]==id)return false;parsed[count].metrics[parsed[count].count++]=id;m=next_token(&metric_cursor,',');}
  ...
  s_watch_hr_to_locus=watch_hr;s_heart_rate_interval=interval;
...
static void format_metric(char *out,size_t n,int m,uint32_t elapsed){
  ...
  else if(m==15||m==16||m==22)snprintf(out,n,"%ld bpm",(long)v);
...
static void write_message(int type,int command){
  DictionaryIterator *it;if(app_message_outbox_begin(&it)!=APP_MSG_OK){s_outbox_busy=false;return;}
  ...
  if(type==MSG_HEART_RATE){dict_write_uint32(it,MESSAGE_KEY_SESSION_ID,s_session_id);dict_write_uint32(it,MESSAGE_KEY_HEART_RATE_SEQUENCE,s_hr_sequence++);dict_write_uint32(it,MESSAGE_KEY_SAMPLE_EPOCH_SECONDS,time(NULL));dict_write_int32(it,MESSAGE_KEY_CURRENT_HEART_RATE,s_pending_hr);s_hr_pending=false;}
  s_outbox_busy=true;app_message_outbox_send();
}
static void send_message(int type,int command){if(type==MSG_COMMAND||!s_pending_message_type){s_pending_message_type=type;s_pending_message_command=command;}send_next();}
static void relay_profile_next(void){size_t length=strlen(s_profile_chunks);if(s_relay_index>=s_relay_count)return;size_t end=s_relay_offset+80;if(end>length)end=length;while(end>s_relay_offset&&end<length&&(((uint8_t)s_profile_chunks[end]&0xc0)==0x80))end--;char part[81];size_t n=end-s_relay_offset;memcpy(part,s_profile_chunks+s_relay_offset,n);part[n]=0;DictionaryIterator *it;if(app_message_outbox_begin(&it)!=APP_MSG_OK)return;dict_write_int32(it,MESSAGE_KEY_PROTOCOL_VERSION,PROTOCOL_VERSION);dict_write_int32(it,MESSAGE_KEY_MESSAGE_TYPE,MSG_PROFILE_LIST_CHUNK);dict_write_int32(it,MESSAGE_KEY_RESULT,s_profile_result);dict_write_cstring(it,MESSAGE_KEY_APP_VERSION,RELEASE_VERSION);dict_write_int32(it,MESSAGE_KEY_TRANSFER_ID,s_relay_id);dict_write_int32(it,MESSAGE_KEY_CHUNK_INDEX,s_relay_index);dict_write_int32(it,MESSAGE_KEY_CHUNK_COUNT,s_relay_count);dict_write_cstring(it,MESSAGE_KEY_CHUNK_DATA,part);s_relay_offset=end;s_relay_index++;s_outbox_busy=true;app_message_outbox_send();}
static void send_next(void){if(s_outbox_busy)return;if(s_relay_index<s_relay_count){relay_profile_next();return;}if(s_pending_message_type){int type=s_pending_message_type,command=s_pending_message_command;s_pending_message_type=0;write_message(type,command);return;}if(s_hr_pending)write_message(MSG_HEART_RATE,0);}
static void relay_profile_list(void){size_t length=strlen(s_profile_chunks),offset=0;if(length)s_profile_result=RESULT_OK;s_relay_count=0;do{size_t end=offset+80;if(end>length)end=length;while(end>offset&&end<length&&(((uint8_t)s_profile_chunks[end]&0xc0)==0x80))end--;offset=end;s_relay_count++;}while(offset<length);s_relay_offset=0;s_relay_index=0;s_relay_id=(int)(time(NULL)&0x7fffffff);send_next();}
static void outbox_sent(DictionaryIterator *it,void *context){s_outbox_busy=false;if(s_request_profiles_after_send&&s_relay_index>=s_relay_count){s_request_profiles_after_send=false;send_message(MSG_REQUEST_PROFILE_LIST,0);}send_next();}
static void outbox_failed(DictionaryIterator *it,AppMessageResult reason,void *context){s_outbox_busy=false;send_next();}
...
#if defined(PBL_PLATFORM_EMERY)
static void health_timeout(void *context){s_health_timeout=NULL;if(s_health_subscribed&&!s_last_hr_sent&&!s_health_notice_shown){s_health_notice_shown=true;snprintf(s_notice,sizeof(s_notice),"%s",tr("No heart rate","Kein Puls verfügbar"));s_notice_until=time(NULL)+5;render();}}
