...
| 7 | watch session ID | Unix seconds when the watchapp opened |
...
| 16 | units | metric `0`, imperial `1` (reserved; v2 emits metric) |
...
Command results are cached by `(session ID, command ID)`, preventing retransmissions within one
session while allowing a reopened watchapp to reuse its command counter safely.
