...
class DiagnosticHistoryTest {
    @Test fun coalescesIdenticalFailuresAndKeepsFirstAndLastTimestamps() {
        var now = 100L
        ...
        now = 250L
        history.record(failure)
        assertEquals(DiagnosticEntry(failure, DiagnosticSeverity.WARNING, 100, 250, 2), history.entries.value.single())
    }

    @Test fun identicalKindWithDifferentDetailRemainsSeparateAndNewestFirst() {
        var now = 1L
        val history = DiagnosticHistory(nowMillis = { now++ })
    ...
    @Test fun retainsOnlyTwentyNewestEntriesAndCanBeCleared() {
    ...
    @Test fun expectedUserActionConditionsAreWarnings() {
        assertEquals(DiagnosticSeverity.WARNING, BridgeFailure(BridgeFailureKind.PEBBLE_COMPANION_NOT_INSTALLED).severity())
        assertEquals(DiagnosticSeverity.ERROR, BridgeFailure(BridgeFailureKind.WATCHAPP_INCOMPATIBLE).severity())
        assertEquals(DiagnosticSeverity.ERROR, BridgeFailure(BridgeFailureKind.SNAPSHOT_DELIVERY_FAILED).severity())
    }
}
