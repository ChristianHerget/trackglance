...
class DebugStatusProvider : ContentProvider() {
    ...
    private fun currentValues(): Map<String, Any?> {
        ...
        return linkedMapOf(
            ...
            "supervision_mode" to live.supervision.settings.mode.name,
            "supervision_phase" to live.supervision.phase.name,
            "supervision_sources" to
                live.supervision.sources.sortedBy { it.name }.joinToString("|") { it.name },
