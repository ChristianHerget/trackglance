            ...
            BridgeState.update { status ->
                status.copy(
                    ...
                    lastError = when (result) {
                        WatchAppLaunchResult.LOOKUP_FAILED -> BridgeFailure(BridgeFailureKind.PEBBLE_WATCH_LOOKUP_FAILED)
                        WatchAppLaunchResult.LAUNCH_FAILED -> BridgeFailure(BridgeFailureKind.WATCHAPP_LAUNCH_FAILED)
                        WatchAppLaunchResult.TIMED_OUT -> BridgeFailure(BridgeFailureKind.WATCHAPP_LAUNCH_TIMED_OUT)
    ...
    private suspend fun refreshDiagnostics() = diagnosticsMutex.withLock {
        ...
        if (!coreSelected && pebbleError == null) {
            pebbleError = when (connection.kind) {
                CoreAppConnectionKind.NOT_INSTALLED -> BridgeFailure(BridgeFailureKind.PEBBLE_COMPANION_NOT_INSTALLED)
                CoreAppConnectionKind.NOT_SELECTED -> BridgeFailure(
                    BridgeFailureKind.PEBBLE_COMPANION_NOT_SELECTED,
                    connection.detail,
                )
                CoreAppConnectionKind.SELECTED -> BridgeFailure(BridgeFailureKind.PEBBLE_COMPANION_NOT_SELECTED)
            }
        }
        ...
        BridgeState.update {
            it.withDiagnosticsSnapshot(
                ...
                ) BridgeFailure(BridgeFailureKind.LOCUS_UNAVAILABLE) else null,
            )
        }
        if (selected != null) {
            try {
            ...
            Column(
                ...
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        StatusLine(stringResource(R.string.pebble_watchapp), if (status.watchAppOpen) stringResource(R.string.open) else stringResource(R.string.closed))
                        StatusLine(stringResource(R.string.pebble_app), status.pebbleAppPackage ?: stringResource(R.string.not_selected))
                        StatusLine(stringResource(R.string.pebble_watch), if (status.watchConnected) stringResource(R.string.connected) else stringResource(R.string.not_connected))
                        StatusLine(
                            stringResource(R.string.watchapp_auto_start),
                            status.watchAppLaunchResult?.localized() ?: stringResource(R.string.not_requested),
                        )
                        StatusLine(
                            stringResource(R.string.watchapp_version),
                            status.watchVersion?.let { version ->
                                if (version == BuildConfig.VERSION_NAME) version
                                else stringResource(R.string.version_expected, version, BuildConfig.VERSION_NAME)
                            } ?: stringResource(R.string.not_reported),
                        )
                        StatusLine(stringResource(R.string.locus_map), if (status.locusAvailable) stringResource(R.string.available) else stringResource(R.string.unavailable))
                        StatusLine(stringResource(R.string.recording), status.recordingState.localized())
                        StatusLine(
                            ...
                            status.locusProfiles?.let { profiles ->
                                ...
                                else stringResource(R.string.profile_count, profiles.size, profiles.joinToString())
                            } ?: stringResource(R.string.not_queried),
                        )
                        StatusLine(stringResource(R.string.last_profile_request), status.lastProfileRequestEpochMillis?.let {
                            DateFormat.getTimeInstance().format(Date(it))
                        } ?: stringResource(R.string.never))
                        StatusLine(stringResource(R.string.last_update), status.lastUpdateEpochMillis?.let {
                            DateFormat.getTimeInstance().format(Date(it))
                        } ?: stringResource(R.string.never))
                        StatusLine(stringResource(R.string.last_watch_hr), status.lastWatchHeartRate?.let { stringResource(R.string.heart_rate_value, it) } ?: stringResource(R.string.never))
                        StatusLine(stringResource(R.string.last_hr_forwarded), status.lastHeartRateForwardedEpochMillis?.let {
                            DateFormat.getTimeInstance().format(Date(it))
                        } ?: stringResource(R.string.never))
                        StatusLine(stringResource(R.string.current_locus_hr), status.currentLocusHeartRate?.let { stringResource(R.string.heart_rate_value, it) } ?: stringResource(R.string.unavailable))
                    }
                }
                Row(
                    ...
                    Text(stringResource(R.string.recent_diagnostics), style = MaterialTheme.typography.titleMedium)
                    Button(onClick = RecentDiagnostics::clear, enabled = diagnosticEntries.isNotEmpty()) {
                        Text(stringResource(R.string.clear))
                    }
                }
                if (diagnosticEntries.isEmpty()) {
                    Text(stringResource(R.string.no_recent_diagnostics))
                } else diagnosticEntries.forEach { entry ->
                    Card(Modifier.fillMaxWidth()) {
                        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                            Text(
                                if (entry.severity == DiagnosticSeverity.WARNING) stringResource(R.string.warning)
                            ...
                            StatusLine(stringResource(R.string.occurrences), entry.count.toString())
                ...
                Text(stringResource(R.string.refresh_title), style = MaterialTheme.typography.titleMedium)
...
@Composable
private fun BridgeProtocol.RecordingState.localized() = stringResource(when (this) {
...
@Composable
private fun RefreshMode.localized() = stringResource(when (this) {
...
@Composable
private fun WatchAppLaunchResult.localized() = stringResource(when (this) {
...
@Composable
private fun BridgeFailure.localizedSummary() = stringResource(when (kind) {
