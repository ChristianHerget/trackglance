...
object PebbleMessages {
    ...
    private fun u(value: Long) =
        PebbleDictionaryItem.UInt32(value.coerceIn(0, UInt.MAX_VALUE.toLong()).toUInt())

    private fun id(value: Long) =
        PebbleDictionaryItem.UInt32(BridgeProtocol.requireUnsigned32(value))
    ...
    fun snapshot(value: BridgeProtocol.Snapshot): PebbleDictionary {
        val sample =
            if (
                ...
                value
            else BridgeProtocol.Snapshot(value.state, value.sampledAtEpochSeconds)
        ...
        val base =
            mapOf(
                ...
                BridgeProtocol.Key.MESSAGE_TYPE.toUInt() to
                    i(BridgeProtocol.MessageType.SNAPSHOT.wire),
                ...
                BridgeProtocol.Key.MOVING_SECONDS.toUInt() to
                    i(nonNegativeLong(sample.movingSeconds)),
                ...
                BridgeProtocol.Key.MOVING_DISTANCE_VALUE.toUInt() to
                    i(display.movingDistance.mantissa),
                ...
                BridgeProtocol.Key.VERTICAL_SPEED_VALUE.toUInt() to
                    i(display.verticalSpeed.mantissa),
                BridgeProtocol.Key.SLOPE_VALUE.toUInt() to i(display.slope.mantissa),
                BridgeProtocol.Key.AVERAGE_HEART_RATE.toUInt() to
                    i(nonNegativeInt(sample.averageHeartRate)),
                BridgeProtocol.Key.MAX_HEART_RATE.toUInt() to
                    i(nonNegativeInt(sample.maxHeartRate)),
                BridgeProtocol.Key.CURRENT_HEART_RATE.toUInt() to
                    i(nonNegativeInt(sample.currentHeartRate)),
                BridgeProtocol.Key.AVERAGE_CADENCE.toUInt() to
                    i(nonNegativeInt(sample.averageCadence)),
                ...
                BridgeProtocol.Key.MOVING_DISTANCE_FORMAT.toUInt() to
                    b(display.movingDistance.format.wire),
                BridgeProtocol.Key.CURRENT_SPEED_FORMAT.toUInt() to
                    b(display.currentSpeed.format.wire),
                BridgeProtocol.Key.AVERAGE_SPEED_FORMAT.toUInt() to
                    b(display.averageSpeed.format.wire),
                BridgeProtocol.Key.MAX_SPEED_FORMAT.toUInt() to b(display.maxSpeed.format.wire),
                BridgeProtocol.Key.VERTICAL_SPEED_FORMAT.toUInt() to
                    b(display.verticalSpeed.format.wire),
    ...
    private fun nonNegativeLong(value: Long?): Int =
        value?.takeIf { it >= 0 }?.coerceAtMost(Int.MAX_VALUE.toLong())?.toInt()
            ?: BridgeProtocol.UNAVAILABLE

    private fun nonNegativeInt(value: Int?): Int =
        value?.takeIf { it >= 0 } ?: BridgeProtocol.UNAVAILABLE

    internal fun encodedSize(dictionary: PebbleDictionary): Int =
        1 +
            dictionary.values.sumOf {
                7 + it.size
            }

    fun result(sessionId: Long, commandId: Long, result: BridgeProtocol.Result): PebbleDictionary =
        mapOf(
            BridgeProtocol.Key.VERSION.toUInt() to i(BridgeProtocol.VERSION),
            BridgeProtocol.Key.APP_VERSION.toUInt() to text(BuildConfig.VERSION_NAME),
            BridgeProtocol.Key.MESSAGE_TYPE.toUInt() to
                i(BridgeProtocol.MessageType.COMMAND_RESULT.wire),
    ...
    fun recordingContext(
        ...
        if (
            profile == null ||
                !BridgeProtocol.validLocusProfileId(profile.id) ||
    ...
    internal fun profileListChunks(
        ...
        require(transferId >= 0)
        return transfer.chunks.mapIndexed { index, chunk ->
            mapOf(
                BridgeProtocol.Key.VERSION.toUInt() to i(BridgeProtocol.VERSION),
                BridgeProtocol.Key.APP_VERSION.toUInt() to text(BuildConfig.VERSION_NAME),
                BridgeProtocol.Key.MESSAGE_TYPE.toUInt() to
                    i(BridgeProtocol.MessageType.PROFILE_LIST_CHUNK.wire),
