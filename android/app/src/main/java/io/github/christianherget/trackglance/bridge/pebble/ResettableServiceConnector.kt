...
/**
 * A cancellable connection cache whose failed or timed-out attempts never poison later sends. Every
 * retry creates a new explicit binding attempt; stale callbacks are rejected by generation.
 */
internal class ResettableServiceConnector<Service>(
    ...
    suspend fun getOrConnect(): Service? = connectMutex.withLock {
        ...
        synchronized(stateLock) {
            ...
            cachedService?.takeIf(isAlive)?.let {
                return@withLock it
            }
        }
        ...
        val callbacks =
            object : ServiceBindingCallbacks<Service> {
                override fun connected(service: Service) {
                    val accepted =
                        synchronized(stateLock) {
                            !closed && generation == token && activeAttempt === attempt
                        }
                    if (accepted && isAlive(service)) result.complete(service)
    ...
    private fun detachDeadCachedService(): ServiceBindingAttempt<Service>? =
        synchronized(stateLock) {
