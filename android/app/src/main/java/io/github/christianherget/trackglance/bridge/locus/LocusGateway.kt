...
sealed interface RecordingProfilesResult {
    data class Success(val profiles: List<BridgeProtocol.RecordingProfile>) : RecordingProfilesResult
...
class LocusGateway(context: Context) : LocusBridgeGateway {
    ...
    companion object {
        @Volatile
        private var processUnitPreferences: LocusUnitPreferencesCache? = null

        private fun sharedUnitPreferences(context: Context): LocusUnitPreferencesCache =
            processUnitPreferences ?: synchronized(this) {
                processUnitPreferences ?: LocusUnitPreferencesCache(
                    read = {
                        val version = LocusUtils.getActiveVersion(context, VersionCode.UPDATE_13)
    ...
    private fun readSnapshot(version: LocusVersion, nowMillis: Long): BridgeProtocol.Snapshot {
        ...
        return BridgeProtocol.Snapshot(
            ...
            currentPaceMinutesPerKilometre = update.pace.takeIf { update.isGpsLocValid && it > 0.0f },
    ...
    override fun recordingProfiles(): RecordingProfilesResult = try {
        val version = activeVersion()
            ?: return RecordingProfilesResult.Failure(BridgeFailure(BridgeFailureKind.LOCUS_UNAVAILABLE))
