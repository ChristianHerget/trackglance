...
object BridgeProtocol {
    ...
    enum class MessageType(val wire: Int) {
        ...
        REQUEST_SNAPSHOT(4),
        CONFIG_CHUNK(5),
        PROFILE_LIST_CHUNK(6),
        REQUEST_PROFILE_LIST(7),
        HEART_RATE_SAMPLE(8),
        CONFIG_RESULT(9),
    ...
    fun validLocusProfileName(name: String?): Boolean =
        validText(
            name,
    ...
    fun validLocusProfileId(id: String?): Boolean =
        id != null &&
            (id == "0" || id.matches(Regex("-?[1-9][0-9]{0,18}"))) &&
            id.toLongOrNull() != null

    fun validWaypointName(name: String?): Boolean =
        validText(
            name,
    ...
    fun validWatchId(value: String?): Boolean =
        validText(
            value,
    ...
    private fun validText(
        ...
        while (index < value.length) {
            ...
            if (codePoint < 0x20 || codePoint == 0x7f || (rejectPipe && codePoint == '|'.code))
                return false
    ...
    private fun unicodeWhitespace(codePoint: Int): Boolean =
        codePoint == 0x20 ||
            codePoint == 0x85 ||
            codePoint == 0xa0 ||
            codePoint == 0x1680 ||
    ...
    fun profileListPayload(profiles: Collection<RecordingProfile>): String? {
        if (
            profiles.any { !validLocusProfileId(it.id) || !validLocusProfileName(it.name) } ||
    ...
    enum class LengthFormat {
        ...
        YARDS,
        YARDS_MILES,
        METRES_NAUTICAL_MILES,
    }

    enum class AltitudeFormat {
        METRES,
        FEET,
    }
