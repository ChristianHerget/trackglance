...
object SnapshotFormatter {
    ...
    fun format(snapshot: BridgeProtocol.Snapshot): DisplaySnapshot {
        ...
        return DisplaySnapshot(
            ...
            altitude =
                fixed(
                    snapshot.altitudeMetres?.times(
                        if (units.altitude == BridgeProtocol.AltitudeFormat.FEET) FEET_PER_METRE
                        else 1.0
                    ),
                    altitudeFormat,
                    allowNegative = true,
        ...
        val descriptor =
            when (preference) {
                BridgeProtocol.LengthFormat.METRES ->
                    Descriptor(metres, BridgeProtocol.FormatCode.M_0)
                BridgeProtocol.LengthFormat.METRES_KILOMETRES ->
                    if (metres != null && metres >= 1000.0) {
                        val kilometres = metres / 1000.0
                        Descriptor(
                            kilometres,
                            if (kilometres >= 100.0) BridgeProtocol.FormatCode.KM_0
                            else BridgeProtocol.FormatCode.KM_1,
                        )
                    } else Descriptor(metres, BridgeProtocol.FormatCode.M_0)
                BridgeProtocol.LengthFormat.FEET ->
                    Descriptor(metres?.times(FEET_PER_METRE), BridgeProtocol.FormatCode.FT_0)
                BridgeProtocol.LengthFormat.FEET_MILES ->
                    imperialDistance(metres, FEET_PER_METRE, BridgeProtocol.FormatCode.FT_0)
                BridgeProtocol.LengthFormat.YARDS ->
                    Descriptor(metres?.times(YARDS_PER_METRE), BridgeProtocol.FormatCode.YD_0)
                BridgeProtocol.LengthFormat.YARDS_MILES ->
                    imperialDistance(metres, YARDS_PER_METRE, BridgeProtocol.FormatCode.YD_0)
                BridgeProtocol.LengthFormat.METRES_NAUTICAL_MILES ->
                    if (metres != null && metres > METRES_PER_NAUTICAL_MILE) {
                        val nauticalMiles = metres / METRES_PER_NAUTICAL_MILE
                        Descriptor(
                            nauticalMiles,
                            if (nauticalMiles >= 100.0) BridgeProtocol.FormatCode.NMI_0
                            else BridgeProtocol.FormatCode.NMI_1,
                        )
                    } else Descriptor(metres, BridgeProtocol.FormatCode.M_0)
            }
            ...
            value
                ...
                ?.times(
                    when (preference) {
                        ...
                        BridgeProtocol.SpeedFormat.KNOTS -> 1.9438444924406046
                    }
                )
        val oneDecimal = converted == null || converted <= 100.0
        val format =
            when (preference) {
                BridgeProtocol.SpeedFormat.KILOMETRES_PER_HOUR ->
                    if (oneDecimal) BridgeProtocol.FormatCode.KPH_1
                    else BridgeProtocol.FormatCode.KPH_0
                BridgeProtocol.SpeedFormat.MILES_PER_HOUR ->
                    if (oneDecimal) BridgeProtocol.FormatCode.MPH_1
                    else BridgeProtocol.FormatCode.MPH_0
                BridgeProtocol.SpeedFormat.NAUTICAL_MILES_PER_HOUR ->
                    if (oneDecimal) BridgeProtocol.FormatCode.NMIH_1
                    else BridgeProtocol.FormatCode.NMIH_0
                BridgeProtocol.SpeedFormat.KNOTS ->
                    if (oneDecimal) BridgeProtocol.FormatCode.KNOT_1
                    else BridgeProtocol.FormatCode.KNOT_0
            }
        return fixed(converted, format)
    }

    private fun slope(value: Float?, preference: BridgeProtocol.SlopeFormat): DisplayValue =
        when (preference) {
            BridgeProtocol.SlopeFormat.PERCENT ->
                fixed(
                    value?.toDouble()?.times(100.0),
                    BridgeProtocol.FormatCode.PERCENT_0,
                    allowNegative = true,
                )
            BridgeProtocol.SlopeFormat.DEGREES ->
                fixed(
                    value?.let { Math.toDegrees(atan(it.toDouble())) },
                    BridgeProtocol.FormatCode.DEGREE_0,
                    allowNegative = true,
                )
        }

    private fun energy(value: Int?, preference: BridgeProtocol.EnergyFormat): DisplayValue =
        when (preference) {
            BridgeProtocol.EnergyFormat.KILOJOULES ->
                fixed(value?.div(1000.0), BridgeProtocol.FormatCode.KJ_0)
            BridgeProtocol.EnergyFormat.KILOCALORIES ->
                fixed(value?.div(JOULES_PER_KILOCALORIE), BridgeProtocol.FormatCode.KCAL_0)
        }
    ...
    private fun wholeSeconds(value: Double?): Int =
        scaled(value, 0, allowNegative = false).takeIf { it > 0 } ?: BridgeProtocol.UNAVAILABLE

    private fun fixed(
        value: Double?,
        format: BridgeProtocol.FormatCode,
    ...
    private fun scaled(value: Double?, decimals: Int, allowNegative: Boolean): Int {
        if (value == null || !value.isFinite() || (!allowNegative && value < 0.0))
            return BridgeProtocol.UNAVAILABLE
    ...
    private fun paceDescriptor(length: BridgeProtocol.LengthFormat): PaceDescriptor =
        when (length) {
            BridgeProtocol.LengthFormat.METRES,
            BridgeProtocol.LengthFormat.METRES_KILOMETRES ->
            ...
            BridgeProtocol.LengthFormat.METRES_NAUTICAL_MILES ->
                PaceDescriptor(METRES_PER_NAUTICAL_MILE, BridgeProtocol.FormatCode.PER_NMI)
        }
